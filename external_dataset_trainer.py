...
def train_from_folder_dataset(dataset_path: str, face_id_system: FaceIDSystem) -> Tuple[int, int]:
 ...
 logger.info(f" [OK] {image_file}")
 else:
 logger.warning(f" [FAIL] {image_file} - No face detected")
 except Exception as e:
 logger.error(f" [FAIL] {image_file} - Error: {e}")
...
def train_from_csv_dataset(csv_path: str, face_id_system: FaceIDSystem) -> int:
 ...
 logger.warning(f"[FAIL] Image not found: {image_path}")
 continue
 ...
 if success:
 successful_registrations += 1
 logger.info(f"[OK] {person_name}: {os.path.basename(image_path)}")
 else:
 logger.warning(f"[FAIL] {person_name}: {os.path.basename(image_path)} - No face detected")
 except Exception as e:
 logger.error(f"[FAIL] {person_name}: {os.path.basename(image_path)} - Error: {e}")
...
def train_from_lfw_dataset(lfw_path: str, face_id_system: FaceIDSystem, max_people: int = 100) -> int:
 ...
 if success:
 successful_registrations += 1
 logger.info(f" [OK] {person_name}")
 else:
 logger.warning(f" [FAIL] {person_name} - No face detected")
 except Exception as e:
 logger.error(f" [FAIL] {person_name} - Error: {e}")
...
def train_with_validation(dataset_path: str, face_id_system: FaceIDSystem, min_face_size: int = 2500) -> int:
 ...
 if not is_valid:
 logger.warning(f"[FAIL] Invalid: {file} (faces: {face_count}, size: {face_size})")
 continue
 
 if face_size < min_face_size:
 logger.warning(f"[FAIL] Too small: {file} (size: {face_size})")
 continue
 ...
 if success:
 successful_registrations += 1
 logger.info(f"[OK] {person_name}: {file}")
 else:
 logger.warning(f"[FAIL] Failed to register: {file}")
 except Exception as e:
 logger.error(f"[FAIL] Error: {file} - {e}")
