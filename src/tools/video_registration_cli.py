...
from src.tools.video_registration import VideoRegistrationSystem

def main():
    """Main function for video registration CLI"""
    print("[VIDEO] Video Registration System")
    print("=" * 50)
    ...
    print("[OK] Face ID System initialized")
    ...
    print("[OK] Video Registration System initialized")
    ...
    while True:
        ...
        if choice == '1':
            ...
            if person_name:
                print(f"\n[VIDEO] Starting video registration for {person_name}")
                ...
                print("[FAIL] Person name is required")
            ...
            if video_path and person_name:
                if os.path.exists(video_path):
                    print(f"\n[VIDEO] Processing video file: {video_path}")
                    success = video_reg.process_video_file(video_path, person_name)
                    
                    if success:
                        print(f"[OK] Video processing successful for {person_name}")
                    else:
                        print(f"[FAIL] Video processing failed for {person_name}")
                else:
                    print(f"[FAIL] Video file not found: {video_path}")
            else:
                print("[FAIL] Both video path and person name are required")
            ...
            if success:
                print("[OK] Recognition test started")
            else:
                print("[FAIL] Failed to start recognition test")
            ...
            print("[FAIL] Invalid choice. Please enter 1-5.")
