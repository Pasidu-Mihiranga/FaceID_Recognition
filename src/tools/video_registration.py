...
from src.processing.augmentation import FaceDataAugmentation
...
class VideoRegistrationSystem:
    ...
    def start_video_registration(self, person_name: str, display_window: bool = True) -> bool:
        ...
        try:
            ...
            print(f"\n[VIDEO] Video Registration for {person_name}")
    ...
    def _process_registration_video(self, person_name: str) -> bool:
        ...
        try:
            ...
            # Extract full frames and bounding boxes for augmentation
            full_frames = [info['frame'] for info in best_frames]
            face_bboxes = [info['face_bbox'] for info in best_frames]
            
            # Apply data augmentation to full frames to prevent double-cropping failures
            logger.info(f"Applying data augmentation to {len(full_frames)} original frames")
            augmented_frames = self.data_augmentation.augment_face_images(full_frames, person_name)
            
            # Reconstruct the corresponding bounding boxes
            aug_factor = len(augmented_frames) // len(full_frames) - 1 if len(full_frames) > 0 else 0
            augmented_bboxes = face_bboxes + [bbox for bbox in face_bboxes for _ in range(aug_factor)]
            
            augmented_face_images = []
            augmented_embeddings = []
            
            logger.info(f"Extracting embeddings and processing {len(augmented_frames)} augmented images")
            
            for i, (aug_frame, bbox) in enumerate(zip(augmented_frames, augmented_bboxes)):
                # 1. Process face with advanced pipeline for the database
                processed_data = self.advanced_processor.process_face_for_recognition(aug_frame, bbox)
                processed_face = processed_data['processed_face']
                augmented_face_images.append(processed_face)
                
                # 2. Extract embedding from FULL frame to prevent ArcFace 'No Face Detected' errors
                try:
                    embedding = self.face_id_system.face_recognizer.recognizer.extract_embedding(aug_frame)
                    augmented_embeddings.append(embedding)
                    if i % 20 == 0:  # Log progress every 20 images
                        logger.info(f"  Processed {i+1}/{len(augmented_frames)} augmented images")
                except Exception as e:
                    logger.warning(f"  Failed to extract embedding for augmented image {i+1}: {e}")
                    
            if len(augmented_embeddings) == 0:
                logger.error("No embeddings extracted from frames")
                return False
                
            # Get augmentation statistics
            aug_stats = self.data_augmentation.get_augmentation_stats(len(full_frames), len(augmented_frames))
            ...
            if identity:
                logger.info(f"[OK] Face identity created for {person_name}")
                ...
                logger.error(f"[FAIL] Failed to create face identity for {person_name}")
                return False
        ...
        try:
            ...
            for i, (face_image, embedding) in enumerate(zip(face_images, face_embeddings)):
                try:
                    ...
                    if success:
                        logger.info(f"  [OK] Traditional registration {i+1} successful")
                    else:
                        logger.warning(f"  [FAIL] Traditional registration {i+1} failed")
                except Exception as e:
                    logger.error(f"  [FAIL] Traditional registration {i+1} error: {e}")
...
if __name__ == "__main__":
    ...
    if video_reg.start_video_registration(person_name):
        print(f"[OK] Video registration successful for {person_name}")
    else:
        print(f"[FAIL] Video registration failed for {person_name}")
