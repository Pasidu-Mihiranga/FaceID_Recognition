...
def test_basic_functionality():
 ...
 logger.error(f"Main system not available: {e}")
 return False
 ...
 logger.info("[OK] Test image created")
 
 # Test face detection
 faces = face_id.face_detector.detect_faces(test_image)
 logger.info(f"[OK] Face detection: Found {len(faces)} faces")
 
 if faces:
 # Test face recognition (should be unknown initially)
 person_name, confidence, face_info = face_id.recognize_face(test_image)
 logger.info(f"[OK] Face recognition: {person_name or 'Unknown'} (confidence: {confidence:.3f})")
 
 # Test person registration
 success = face_id.register_person(test_image_path, "TestPerson")
 logger.info(f"[OK] Person registration: {'Success' if success else 'Failed'}")
 
 # Test recognition after registration
 person_name, confidence, face_info = face_id.recognize_face(test_image)
 logger.info(f"[OK] Recognition after registration: {person_name} (confidence: {confidence:.3f})")
 
 # Test database operations
 persons = face_id.database.get_all_persons()
 logger.info(f"[OK] Database: {len(persons)} persons registered")
 
 # Test system stats
 stats = face_id.get_system_stats()
 logger.info(f"[OK] System stats: {stats['database_stats']['total_persons']} persons")
...
def test_imports():
 ...
 import pickle
 import json
 logger.info("[OK] Core libraries imported")
 
 # Test optional ML libraries
 try:
 import tensorflow as tf
 logger.info("[OK] TensorFlow imported")
 except ImportError:
 logger.warning("[WARN] TensorFlow not available")
 
 try:
 import torch
 logger.info("[OK] PyTorch imported")
 except ImportError:
 logger.warning("[WARN] PyTorch not available")
 
 try:
 from sklearn.cluster import DBSCAN
 logger.info("[OK] Scikit-learn imported")
 except ImportError:
 logger.warning("[WARN] Scikit-learn not available")
 
 # Test face recognition libraries
 try:
 import mtcnn
 logger.info("[OK] MTCNN imported")
 except ImportError:
 logger.warning("[WARN] MTCNN not available")
 
 try:
 import insightface
 logger.info("[OK] InsightFace imported")
 except ImportError:
 logger.warning("[WARN] InsightFace not available")
 
 try:
 import deepface
 logger.info("[OK] DeepFace imported")
 except ImportError:
 logger.warning("[WARN] DeepFace not available")
 
 # Test web framework
 try:
 import flask
 logger.info("[OK] Flask imported")
 except ImportError:
 logger.warning("[WARN] Flask not available")
...
def test_directory_structure():
 ...
 logger.error(f"[FAIL] Missing directory: {dir_path}")
 return False
 ...
 logger.error(f"[FAIL] Missing file: {file_path}")
 return False
 logger.info(f"[OK] File exists: {file_path}")
 
 logger.info("[OK] Directory structure is correct")
 return True
 
 except Exception as e:
 logger.error(f"[FAIL] Directory structure test failed: {e}")
 return False
